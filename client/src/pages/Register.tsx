...
export default function Register() {
  ...
  // Read returnTo from query string (e.g. /register?returnTo=/impact)
  const returnTo = new URLSearchParams(window.location.search).get("returnTo") || "/dashboard";

  const registerMutation = trpc.auth.register.useMutation({
    onSuccess: (data) => {
      localStorage.setItem("ayah_user", JSON.stringify(data.user));
      toast.success(`Welcome, ${data.user.name ?? "there"}! Your account has been created.`);
      navigate(returnTo);
    },
    onError: (err) => {
  ...
  const passwordStrength = () => {
    ...
    if (password.length < 8) return { level: "weak", color: "text-red-400", label: "Too short" };
    if (password.length < 12 && !/[A-Z]/.test(password)) return { level: "fair", color: "text-yellow-400", label: "Fair" };
    if (password.length >= 12 && /[A-Z]/.test(password) && /[0-9]/.test(password)) return { level: "strong", color: "text-green-400", label: "Strong" };
      ...
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          <Link href="/" className="inline-flex items-center gap-2 text-2xl font-bold text-white">
            ...
            DataMind <span className="text-violet-400">AI</span>
          </Link>
          <p className="text-slate-400 mt-2 text-sm">Your AI-Powered Data Consultant</p>
        </div>
        ...
        <Card className="bg-slate-800/50 border-slate-700 shadow-2xl">
          <CardHeader className="text-center pb-4">
            ...
            <CardDescription className="text-slate-400">
              Sign up to save your chat history and get more messages
            </CardDescription>
          </CardHeader>
          <CardContent>
            ...
            <div className="relative mb-5">
              ...
              <div className="relative flex justify-center text-xs">
                <span className="bg-slate-800 px-3 text-slate-400">or sign up with email</span>
              </div>
            </div>
