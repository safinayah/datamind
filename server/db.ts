...
export async function getApprovedTestimonials(type?: "personal" | "tool") {
  const db = await getDb();
  if (!db) return [];
  if (type) {
    return db.select().from(testimonials).where(
      and(eq(testimonials.approved, true), eq(testimonials.testimonialType, type))
    );
  }
