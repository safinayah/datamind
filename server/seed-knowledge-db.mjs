/**
 * Direct DB seed script for knowledge base articles.
 * Inserts articles directly into the DB, then triggers chunking via the rag module.
 * Run: node server/seed-knowledge-db.mjs
 */
...
Gartner Data and Analytics Maturity Model (5 levels):
...
async function seed() {
  ...
  for (const article of articles) {
    try {
      const [result] = await conn.execute(
        `INSERT INTO knowledgeArticles (title, topic, source, sourceUrl, content, isActive)
         VALUES (?, ?, ?, ?, ?, 1)`,
        [article.title, article.topic, article.source, article.sourceUrl, article.content]
      );
  ...
  await conn.end();
  console.log(`\nDone. ${inserted}/${articles.length} articles inserted.`);
  console.log("Note: Embeddings will be generated when admin triggers 'Re-ingest' in the Knowledge Base panel.");
}
